...
void RC4_KSG(long dataLen, unsigned char* key, long keyLen,unsigned char* result)
/* Function to return KeyStrean generated for different plain text len*/
{
    unsigned char T[256];
    unsigned char S[256];
    unsigned char  tmp; // to be used in swaping
    ...
    for(int x = 0 ; x< dataLen ; x++)
    {
        ...
        result[x]= 0^S[t]; // XOR generated S[t] with Byte from the plaintext / cipher and append each Encrypted/Decrypted byte to result array
    }
}
